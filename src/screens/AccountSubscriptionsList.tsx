/**
 * Nested list — Billing subscriptions under an account.
 */
...
import { NESTED_SUBSCRIPTION_LIST_VIEW_CHIPS } from '../data/nestedListViewChips'
...
export default function AccountSubscriptionsList() {
  ...
  const chips = NESTED_SUBSCRIPTION_LIST_VIEW_CHIPS.map((c) => ({ ...c }))

  return (
    <NestedDetailViewRoot dataName="AccountSubscriptionsList">
      <NestedDetailViewHeaderStack>
        <NestedPageHeader breadcrumbs={breadcrumbs} title="Subscriptions" />
      ...
      <ListViewBody className="pb-9 pt-9">
        <div className="flex min-w-0 max-w-[1120px] flex-1 flex-col gap-4">
          ...
          <InlineListPagination
            pageStart={1}
            pageEnd={ROWS}
            totalResults={INLINE_LIST_TOTALS.subscriptions}
          />
        </div>
      </ListViewBody>
    </NestedDetailViewRoot>
