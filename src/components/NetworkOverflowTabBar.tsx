/**
 * Network parent list — primary audience tabs with overflow (“More ▾”), matching {@link TransactionsOverflowTabBar}.
 */

import { useLayoutEffect, useRef, useState, useCallback } from 'react'
import ChevronDownIcon from '../icons/ChevronDownIcon'
import { NETWORK_PAGE_TABS, type NetworkTabId } from '../data/networkAudience'

const TAB_GAP_PX = 16
const MORE_BUTTON_RESERVE_PX = 96
const TABLIST_MORE_GAP_PX = 8

type TabDef = (typeof NETWORK_PAGE_TABS)[number]
...
type NetworkOverflowTabBarProps = {
  activeTab: NetworkTabId
  onTabChange: (tabId: NetworkTabId) => void
}

export default function NetworkOverflowTabBar({ activeTab, onTabChange }: NetworkOverflowTabBarProps) {
  const tabs = NETWORK_PAGE_TABS
  ...
  const renderTabButton = (tab: TabDef, isActive: boolean, opts: { measure?: boolean }) => (
    <button
      ...
      className="group relative flex h-fit shrink-0 cursor-pointer items-center pb-3 pt-3 font-label-medium-emphasized text-[14px] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-action-primary"
      style={{
  ...
  return (
    <div className="flex w-full min-w-0 shrink-0 flex-col" data-name="Tabs">
        ...
        {showMore ? (
          <div className="relative shrink-0" ref={menuRef}>
            <button
              ...
              className={`relative flex shrink-0 items-center gap-1 rounded-[var(--radius-action)] px-2 py-1 pb-3 pt-3 font-label-medium-emphasized text-[14px] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-action-primary ${
            ...
            {menuOpen ? (
              <ul
                role="menu"
                className="absolute right-0 top-full z-20 mt-1 max-h-[min(400px,70vh)] min-w-[220px] overflow-auto rounded-[12px] border border-neutral-50 bg-surface py-1 shadow-[0px_4px_16px_rgba(48,49,61,0.12)]"
              >
      ...
      <div className="-mt-px h-px w-full shrink-0 bg-neutral-50" aria-hidden />
    </div>
  )
}
